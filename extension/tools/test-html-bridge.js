...
function host(){
 ...
 sendMessage:async(id,m)=>{calls.push(['message',id,m]);if(failTarget&&id===2)throw Error('target unavailable');return {ok:true,htmlBridgeVersion:'1.4.0'};}},
...
(async()=>{
 ...
 await test('Window command queries and opens HTML main world, not target renderer',async()=>{
  ...
  const status=await h.send({type:'DUO_GET_STATE'});assert.equal(status.workerVersion,'1.4.0');assert(status.overlay);assert(!('captionWindow' in status));
