...
var APP_VERSION = 'v1.49.3';
var APP_BUILD = '20260923-v1493-extension-transport';
  ...
  ROUTES:{
    'jev-direct':{
      ...
      body:function(state,model){
        return {model:model,state:state,questions:TurnProviders.questions(state)};
      },
    ...
    'jev-extension':{
      ...
      defaultModel:'jev-latest',
      keyHint:'TypeSafe の API キー',
      contextNote:'64k context・アドオンが往復を肩代わりします',
      capabilities:{probabilitySemantics:'native_calibrated',supportsChoice:true,supportsNoul:true,
        structuredOutput:true,abortable:true,maxChoices:255,textOnly:true,local:false},
      path:'/v1/systemone',
      extraHeaders:false,
    ...
    'jev-openrouter':{
      ...
      /* 公式が「OpenRouter 経由では 32k」と書いているので、直叩きより文脈を削る。 */
      contextNote:'32k context（直接の半分）',
      maxStateChars:24000,
      ...
      body:function(state,model){
        return {model:model,temperature:0,
          ...
          messages:[{role:'system',content:TurnProviders.ENVELOPE_PROMPT},
            {role:'user',content:JSON.stringify({state:state,questions:TurnProviders.questions(state)})}]};
      },
  ...
  httpProvider:function(id){
    ...
    return {
      ...
      evaluate:function(state,opts){
        ...
        var model=String(CFG.turnDecisionModel||'')||route.defaultModel;
        /* active では alias ではなく校正済み version を pin する（§8.5）。 */
        if(TurnDecision.applies(state.sourceLanguage)&&/latest/i.test(model))
          return Promise.reject(new Error('active では model に固定versionを指定してください（いまは '+model+'）'));
        ...
        /* 経路ごとの文脈上限。超えたら送らずRulesへ落とす。途中で切ると
           候補の offset と本文がずれ、存在しない位置を選ばせてしまう。 */
        if(route.maxStateChars&&body.length>route.maxStateChars)
