...
var STT_ADAPTER_CAPABILITIES={
  /* Web SpeechはChrome 135+のSpeechRecognition.start(audioTrack)で共有音声Trackを
     直接認識できる。WebSpeechTrackEngineがその経路の実装であり、開始できない実機は
     エンジン側が理由を出して止まる。テーブルで先に塞がない。 */
  webspeech:{microphone:true,systemAudio:true,arbitraryTrack:true},
...
function duoValidateInputs(){
  ...
  /* endpointが有効かどうかではなく、A／Bのどちらかが認識源としてタブ／システム
     音声に束ねられているかで判定する。会議音声をTTSの送出先にしか使わない構成を
     巻き込んで止めないため。 */
  var recognisingShared=['A','B'].some(function(view){return AudioEndpointManager.legacySource(view)==='display';});
  if(!recognisingShared)return true;
  /* 経路ごとに必要な能力が違う。directはTrackを直接渡し、apiはBlobを送る。
     vbは仮想入力デバイスなので通常のマイク扱いで、どのadapterでも通る。 */
  var route=effectiveDisplaySttRoute();
  if(route==='direct'&&!caps.arbitraryTrack){
    toast('この音声認識では共有音声Trackを直接認識できません。<br>⚙→音声 で共有音声の認識経路を「VB-CABLE」または「外部API」にしてください。');return false;
  }
  if(route==='api'&&!caps.systemAudio){
    toast('共有音声を外部APIで認識するには OpenAI / xAI / Groq / Gemini のいずれかを選んでください。<br>Web Speechは共有音声Trackの直接認識（⚙→音声）で使えます。');return false;
  }
  return true;
}
...
async function duoConferenceCommand(data){
  ...
  try{
    ...
    if(pc!==duoConferencePeer)return;
    /* Arm the deadline only once the offer is on its way. Starting it before this
       side's ICE gathering made one 15s budget cover two gathering phases - ours
       and the answerer's, up to 8s each - so the handshake could expire before the
       answer was even sent, with no error from either side to explain it. */
    duoConferenceTimer=setTimeout(function(){if(pc===duoConferencePeer)duoConferenceStop('connection-timeout');},15000);
    duoConferenceSend({kind:'offer',token:data.token,description:pc.localDescription.toJSON()});
...
/* 拡張の「Web会議マイク音声」ボタン1つで、字幕対象・HTML本体・翻訳開始・会議送出まで
   通すための入口。押した瞬間に開始まで済ませたいので、結果を返せる形にしておく
   （コマンド経路は投げっぱなしで、開始できたかどうかが分からない）。

   マイクの許可はこのオリジンに保存されているので、利用者の操作が無くても
   getUserMedia は通る。AudioContext も、マイクを許可したオリジンでは
   自動再生規制の対象外になる。

   presetには触れない。duoApplyPreset は実行中なら stopAll して会議送出も切るので、
   ここで当て直すと「開始するために止める」ことになる。利用者が選んだ構成
   （many_to_many など）もボタン1つで書き換わってしまう。 */
function duoExtensionStart(){
  if(S.running)return {ok:true,running:true,already:true};
  try{ startAll(); }
  catch(err){ return {ok:false,running:false,error:String(err.message||err)}; }
  if(!S.running)return {ok:false,running:false,
    error:'開始条件が足りません。HTML本体のタブを開いて、画面に出ている案内をご確認ください'};
...
/* 判断層の設定UI。extension/app.html と index.html の両方へ同じ markup を置くと
   3ファイル目の同期先が増えるので、duoNextInstall と同じくJSから注入する。
   bindSettings() は読み込み時に一度だけ走るため、ここで simple() を呼んで
...
function turnDecisionInstall(){
  ...
  box.innerHTML='<summary>発話交代の判断層（実験）</summary>'
    ...
    +'</select></label>'
    +'<label>日本語 <select id="turnDecisionLangJa">'
      +'<option value="off">off</option><option value="shadow">shadow</option>'
      +'<option value="assist">assist</option><option value="active">active</option>'
    +'</select></label>'
    ...
    +'</details>'
    +'<label>「待て」に従う上限 ms（0＝従わない） <input id="turnDecisionHoldMaxWaitMs" type="number" min="0" max="10000" step="100"></label>'
    +'<details class="settings-help"><summary>なぜ上限が必要か</summary>'
      +'<p>判断層が「まだ切るな」と答えた場合、その答えは<b>同じ文の同じ版に対して'
      +'ずっと有効なまま</b>です。認識が止まるとテキストの版は進まないので、'
      +'止まった瞬間の「待て」を読み直し続け、そのカードは確定も翻訳も読み上げも'
      +'されずに残ります（v1.49.19の実測で、停止まで112秒・83秒放置されたカードが'
      +'各記録に1〜2件）。上限を過ぎたらRulesが確定させます。'
      +'時計は確定済みの部分ごとに持つので、話が続いて文が伸びても止まりません。</p>'
    +'</details>'
    +'<label>同じ版を聞き直す上限回数 <input id="turnDecisionMaxAsksPerRevision" type="number" min="1" max="20" step="1"></label>'
    +'<details class="settings-help"><summary>費用に直接効きます</summary>'
      +'<p>質問は「テキスト・候補・無音の長さ」が変わるたびに新しい状態として飛びます。'
      +'無音は100ms刻みで数えるので、<b>テキストが止まったまま無音だけが伸びる状態が'
      +'延々と新しい質問になっていました</b>（v1.49.19の実測で、内容の同じ往復が'
    ...
    +'</select></label>'
    +'<label class="settings-check switch"><input type="checkbox" id="turnFloorSafeToSpeak">'
      +' 読み上げ開始にも判断層を使う（safe_to_speak）</label>'
    +'<details class="settings-help"><summary>読み上げ開始の判断について</summary>'
      +'<p>commit のときに同じ1リクエストで既に答えをもらっているので、'
      +'<b>APIの費用は増えません。</b></p>'
      +'<p><b>「話者との重なり」を「許容」にしているあいだは働きません。</b>'
      +'同時通訳は元の話者に重ねて読むのが普通で、それは利用者の明示的な選択だからです。'
      +'効くのは「避ける」と「自動」、つまり無音を待つとルールが決めた場面だけで、'
      +'そこでは無音の秒数より判断層のほうが確かな根拠になります。</p>'
      +'<p>保持は上の「読み上げ開始の待ち上限」と、答えの鮮度（2秒）の'
      +'<b>短いほう</b>で頭打ちになります。</p>'
    +'</details>'
    +'<hr><p><b>計測</b>　判断層とは独立に動きます。OFF のままでも記録できます。</p>'
    +'<label>trace 記録 <select id="turnTraceMode">'
  ...
  ['turnDecisionMode','turnDecisionLangEn','turnDecisionLangJa','turnDecisionProsody',
   ...
   'turnTraceMode','turnDecisionRawLog'].forEach(function(prop){
    ...
    if(spec.type==='bool')$(spec.el).checked=!!CFG[prop];else $(spec.el).value=CFG[prop];
    simple(spec);
  });

  /* キーは経路の vendor ごとに持つ。経路を変えたら、その vendor のキーを出す。
     値は表示しない。設定済みかどうかと文字数だけ見せる。 */
  function routeOf(){
    var id=String(CFG.turnDecisionProvider||'rules'),list=TurnProviders.list(),i;
  ...
  $('turnDecisionKey').onchange=function(){
    ...
    if(!v)return;                       /* 空欄は「変更しない」。消すのは下のボタン */
    TurnProviders.setKey(r.vendor,v);this.value='';keyRefresh();
    toast(r.label+' のキーを保存しました。');
  };
  /* simple() の onchange がこの前に登録されるので、ここへ来た時点で CFG は更新済み。
     遅らせる必要はなく、遅らせると切り替えた直後の1瞬だけ前の経路の表示が残る。 */
  $('turnDecisionProvider').addEventListener('change',keyRefresh);
  keyRefresh();

  $('turnTraceExport').onclick=function(){
    if(!TurnTrace.rows.length){toast('記録された trace がありません。<br>trace 記録を record にして会議を行ってください。');return;}
    var name='turn-trace-'+new Date().toISOString().replace(/[:.]/g,'-')+'.json';
    var blob=new Blob([TurnTrace.export()],{type:'application/json'});
    var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;a.click();
    setTimeout(function(){URL.revokeObjectURL(a.href);},10000);
    dlog('segment','turn-trace-export',{rows:TurnTrace.rows.length,dropped:TurnTrace.dropped,file:name});
    toast(TurnTrace.rows.length+'行を書き出しました。<br>node extension/tools/turn-trace-replay.js '+name);
  };
  $('turnDecisionKeyClear').onclick=function(){
    var r=routeOf();if(r.local){toast('この経路にキーはありません。');return;}
    TurnProviders.setKey(r.vendor,'');keyRefresh();
    toast(r.label+' のキーを消しました。');
  };
  /* 会議を1本回さないと到達可否が分からない状態だったので、1クリックで試せるようにする。
     判断層のモードには触れない。off のままでも試せる。 */
  $('turnDecisionProbe').onclick=function(){
    var r=routeOf(),btn=this;
    if(r.local){toast(r.label+' は外部へ送らないので、疎通の確認は要りません。');return;}
    if(!TurnProviders.keyFor(r.vendor)){toast('先に '+r.label+' のキーを入れてください。');return;}
    btn.disabled=true;toast('疎通を試しています…');
    TurnProviders.probe(r.id).then(function(res){
      btn.disabled=false;
      if(res.ok){toast('到達しました（'+res.ms+'ms）。<br>'+r.label+' は使えます。');return;}
      if(res.needsBridge){
        toast('アドオンを経由できません（'+res.ms+'ms）。<br>'+res.detail+'<br>'
          +'拡張版では不要ですが、HTML本体ではポップアップの「HTML本体を開く」で'
          +'先に接続してください。');
        return;
      }
      if(res.needsSetup){
        toast('アドオンの接続先が未許可です（'+res.ms+'ms）。<br>'
          +'Duo のポップアップの「判断層（Jev）の接続を許可」を押してください。');
        return;
      }
      if(res.unreachable){
      ...
      toast('サーバまでは届きましたが拒否されました（HTTP '+res.status+'）。<br>'
        +(res.status===401||res.status===403?'キーをご確認ください。'
          :res.status===404?'Base URL と経路をご確認ください。':'応答: '+(res.detail||'')));
    },function(err){btn.disabled=false;toast('確認できませんでした: '+((err&&err.message)||err));});
  };
  $('turnTraceClear').onclick=function(){
    var n=TurnTrace.rows.length;TurnTrace.reset();turnDecisionStatus();
    toast(n?n+'行の trace を破棄しました。':'trace は空です。');
  };
  setInterval(turnDecisionStatus,1000);turnDecisionStatus();
}
/* 現在の状態を1行で出す。OFFで沈黙していることを目で確かめられるようにする。 */
function turnDecisionStatus(){
  var el=$('turnDecisionStatus');if(!el)return;
  var mode=CFG.turnDecisionMode||'off';
  var parts=['判断 '+mode,'経路 '+(CFG.turnDecisionProvider||'rules')];
  if(mode!=='off')parts.push('en='+(CFG.turnDecisionLangEn||'shadow')+' / ja='+(CFG.turnDecisionLangJa||'shadow'));
  parts.push('trace '+(String(CFG.turnTraceMode||'off')==='record'?TurnTrace.rows.length+'行'+(TurnTrace.dropped?'（上限超過あり）':''):'off'));
  var st=TurnDecision.stats||{};
  if(mode!=='off')parts.push('往復 送信'+(st.requested||0)+'/応答'+(st.answered||0)
    +'/解析失敗'+(st.parseFailed||0)+'/到達不能'+(st.unreachable||0)+'/適用'+(st.applied||0)
    +'/打切り'+(st.holdExpired||0));
  parts.push('確定 rules '+(SEG.committedBySource.rules||0)+' / provider '+(SEG.committedBySource.provider||0));
  parts.push('確定後訂正 rules '+(SEG.correctedBySource.rules||0)+' / provider '+(SEG.correctedBySource.provider||0));
  var warn=turnDecisionModelWarning();
  if(warn)parts.push('⚠ '+warn);
  el.textContent=parts.join(' ／ ');
}
/* 経路の既定を含めた「実際に送る model」。 */
function turnDecisionEffectiveModel(){
  var id=String(CFG.turnDecisionProvider||'rules'),list=TurnProviders.list(),i,r=null;
  for(i=0;i<list.length;i++)if(list[i].id===id)r=list[i];
  return String(CFG.turnDecisionModel||'')||(r&&r.defaultModel)||'';
}
/* alias のまま assist／active にすると httpProvider がその場で全部拒否する。
   v1.49.17 の実測では「送信13／応答0／適用0」で Rules が全部を運んでいたのに、
   その理由は診断ログの中だけにあった。1秒ごとに目に入る場所へ出す。 */
function turnDecisionModelWarning(){
  var m=turnDecisionEffectiveModel();
  if(!m||!/latest|preview/i.test(m))return '';
  var langs=[];
  if(TurnDecision.applies('en'))langs.push('英語');
  if(TurnDecision.applies('ja'))langs.push('日本語');
  if(!langs.length)return '';
  return langs.join('・')+'が '+TurnDecision.modeFor(langs[0]==='英語'?'en':'ja')
    +' ですが model が '+m+'（alias）なので送信は毎回拒否されます。jev-1.13.0 のように固定versionを入れてください。';
}
...
var CONFIG_SCHEMA = [
  ...
  { prop:"turnDecisionMode", key:'di.tdMode', embed:'turnDecisionMode', def:'off', portable:true, el:"turnDecisionMode" },
  { prop:"turnDecisionProvider", key:'di.tdProv', embed:'turnDecisionProvider', def:'rules', portable:true, el:"turnDecisionProvider" },
  { prop:"turnDecisionModel", key:'di.tdModel', embed:'turnDecisionModel', def:'', portable:true, el:"turnDecisionModel" },
  { prop:"turnDecisionBaseUrl", key:'di.tdBase', embed:'turnDecisionBaseUrl', def:'', portable:true, el:"turnDecisionBaseUrl" },
  { prop:"turnDecisionApiKey", key:'di.tdKey.local', def:'', localOnly:true, portable:false, note:"判断層のAPIキー（旧・単一保存）。vendor別へ移行済みで読み出しの後方互換にだけ使う" },
  /* vendor ごとのキー。TypeSafe直・OpenRouter・将来のOpenAI/Claudeで別々のキーが要る。
     1本の欄を使い回すと経路を切り替えるたび貼り直しになるので、vendor名→キーのmapで持つ。
     embed を持たないので設定埋込みHTMLには出ない。portable:false で書き出しにも乗らない。 */
  { prop:"turnDecisionKeys", key:'di.tdKeys.local', def:'', localOnly:true, portable:false, note:"判断層のvendor別APIキー。翻訳用キーと分離し書き出さない" },
  ...
  { prop:"turnDecisionLangEn", key:'di.tdEn', embed:'turnDecisionLangEn', def:'shadow', portable:true, el:"turnDecisionLangEn" },
  { prop:"turnDecisionLangJa", key:'di.tdJa', embed:'turnDecisionLangJa', def:'shadow', portable:true, el:"turnDecisionLangJa" },
  ...
  { prop:"turnFloorSafeToSpeak", key:'di.tdFloorS2S', embed:'turnFloorSafeToSpeak', def:'1', type:'bool', portable:true, el:"turnFloorSafeToSpeak" },
  { prop:"turnFloorExpiry", key:'di.tdFloorExp', embed:'turnFloorExpiry', def:'speak', portable:true, el:"turnFloorExpiry" },
  ...
  /* 原文（併記されるほう）の大きさを訳文比の％で持つ。以前は .bubble .sub が50%、
     タブ内字幕が58%、別ウィンドウ字幕が65%と面ごとに違う値を直書きしていた。
     3面ともこの1つの数字を見る。 */
  { prop:"srcRatio", key:'di.srcratio', embed:'srcRatio', def:'60', coerce:function(raw){ var n=parseInt(raw,10); return isFinite(n) ? String(Math.max(40,Math.min(100,Math.round(n/5)*5))) : '60'; }, portable:true, el:"srcRatio", bind:'custom' },
  ...
  { prop:"aivisPrefetch", key:'di.aipf', embed:'aivisPrefetch', def:'on', coerce:function(raw){ return raw==='off' ? 'off' : 'on'; }, portable:true, el:"aivisPrefetch" },
  ...
  try{
    ...
    style.textContent=`
      *{box-sizing:border-box}html,body{height:100%;margin:0}body{display:flex;flex-direction:column;background:#111822;color:#edf3fc;font:14px/1.4 system-ui,sans-serif;color-scheme:dark}
      ...
      .meta{font-size:11px;opacity:.7;margin-bottom:4px}.main{font-size:var(--caption-font);white-space:pre-wrap;opacity:var(--text-alpha)}.sub{font-size:calc(var(--caption-font)*var(--src-ratio,.6));white-space:pre-wrap;margin-top:6px;opacity:var(--text-alpha)}.empty{color:#b8c7da;padding:18px 6px;font-size:14px}footer{flex:none;padding:5px 10px;font-size:11px;color:#b8c7da;background:#1c293b}
    `;
...
function renderCaptionPip(){
  ...
  style.setProperty('--src-ratio',String(srcRatioNum()/100));
...
/* =========================================================================
   ...
   ========================================================================= */
/* 800件では7分の会議で最初の6分が消えていた。「1箇所だけ読み上げが始まらない」の
   原因（audio-cancel の why）が窓の外に落ちるため、原因を出せない診断になる。
   1件はおよそ200バイトで、3000件でも600KB程度。 */
var DLOG = [], DLOG_MAX = 3000, DLOG_T0 = Date.now();
...
function aivisSpeakBlob(text, lang, seat, prosody){
  ...
  aivisLimitedFetch(AIVIS_URL, {
    ...
    /* 上限に当たった発話も捨てずに読む。黙って飛ばすと、聞き手には無音の理由が
       分からない。finish() はブラウザ再生の完了で呼ぶ（先に呼ぶと音が重なる）。 */
    if(err.status===429){
      /* 合成用に張ってあった時計を外す。残り時間のほうが短いと、ブラウザ再生の
         途中で guard が finish() を呼んで次の発話と重なる。ブラウザ側は自前の
         watchdog を持っているので、解放が取り残されることはない。 */
      finish.holdPhase('browser-fallback');
      ttsFallback(text,lang,'rate-limit','Aivisの利用上限に達しました。',
        'aivisRateFallbackWarned',prosody,function(){finish();});
      setTimeout(function(){window.aivisRateFallbackWarned=false;},30000);
      return;
    }
    finish();
...
function aivisSpeakStream(text, lang, seat, prosody){
  ...
  /* 先読み（aivisPrefetchTick）が見る「この音があと何ms鳴るか」。受信が終わるまでは
     長さが決まらないので、recvDone が立つまで先読みしない。 */
  var tail = { gen: myGen, recvDone: false, remainMs: function(){ return Math.max(0, (next - ctx.currentTime) * 1000); } };
  AIVIS_STREAM_TAIL = tail;
  var streamEnd = function(why){
    if (streamDone) return; streamDone = true;
    if (AIVIS_STREAM_TAIL === tail) AIVIS_STREAM_TAIL = null;
    clearTimeout(streamEndTimer); clearTimeout(streamWatchdog);
    if (ttsCurEnd === streamEnd) ttsCurEnd = null;
    ttsPlayingNow = false;
    aivisAbort = null;
  ...
  var drain = function(flush){
    ...
    if (!started){
      started = true; waMode = true; firstMs = Date.now() - t0;
      dlog('tts','aivis-first',{ ms: firstMs, leadMs: Math.round(LEAD*1000), rate: head.rate, ch: head.ch, style: stLog, prefetched: !!pre });
    }
  };

  var bodyJson = JSON.stringify(body), pre = aivisPrefetchTake(bodyJson);
  /* 先読みした要求は送信済みなので、送信時に張るはずだった時計をここで張る。 */
  if (pre){ finish.armPhase('synthesis', guardMs); armStreamWatchdog(); }
  (pre ? pre.promise : aivisLimitedFetch(AIVIS_URL, {
    method:'POST', signal: ctrl && ctrl.signal,
    headers:{ 'Content-Type':'application/json', 'Authorization':'Bearer ' + aivisKey() },
    body: bodyJson
  },{finish:finish,guardMs:guardMs,wait:function(){clearTimeout(streamWatchdog);},send:armStreamWatchdog})).then(function(r){
    var meta=aivisBillingMeta(r);aivisLogBilling(meta,pre?'stream-prefetch':'stream');
    if (!r.ok) return r.text().then(function(x){ throw aivisHttpError(r,x); });
    if (!r.body || !r.body.getReader) throw new Error('ストリーミング非対応');
    var reader = r.body.getReader();
    var pump = function(){
      return reader.read().then(function(res){
        ...
        if (res.done){
          ...
          var remainMs = endAfterBuffered('ended');
          dlog('tts','aivis-stream-ok',{ chars:text.length, firstMs:firstMs, totalMs:Date.now()-t0, prefetched:!!pre,
    ...
    if (started){ endAfterBuffered('failed'); return; }
    /* まだ音が出ていない上限失敗は、ブラウザ内蔵音声で読んでから解放する。
       streamEnd を先に呼ぶと読み上げスロットが空いて次の発話と重なる。 */
    if(err.status===429){
      clearTimeout(streamWatchdog);finish.holdPhase('browser-fallback');
      ttsFallback(text,lang,'rate-limit','Aivisの利用上限に達しました。',
        'aivisRateFallbackWarned',prosody,function(){streamEnd('rate-limit');});
      setTimeout(function(){window.aivisRateFallbackWarned=false;},30000);
      return;
    }
    streamEnd('failed');
...
var TTS_PROVIDERS = {
  ...
  aivis: {
    ...
    /* こちら側の数えは予測にすぎない。ブラウザ内蔵音声へ回すかは aivisOverLimit
       に従う（既定は回さない＝超過分はAivis側の設定＝クレジットに任せる）。 */
    rateWait:function(){return aivisFallbackWait();},
  ...
  if (af){
    ...
    /* 予測（こちら側の数え）と事実（Aivisの429）を1行で並べる。声が替わった理由を
       後から追えるようにするため、どちらで止まったのかが分かる形にしておく。 */
    {section:"settings",order:46.2,label:'Aivis 60秒10回を超えたとき',value:function(){
      var srv=aivisServerWait(),win=aivisWindowWait();
      return {send:'そのまま送る（超過分はAivis側の設定に任せる）',browser:'ブラウザ内蔵音声で読む',wait:'枠が空くまで待つ'}[aivisOverLimitMode()]
        +' ／ 直近60秒 '+AIVIS_RATE.sent.length+'回'
        +(srv>0?' ／ Aivisが429で停止中 あと'+(srv/1000).toFixed(1)+'秒'
          :win>0?' ／ 予測では枠切れ あと'+(win/1000).toFixed(1)+'秒':' ／ 送信可能'); },inactive:'(未使用)'},
    /* 先読みが効いているか。使った数と捨てた数を並べる。捨てた分は60秒10回の枠を
       余分に使っているので、理由ごとに分ける。 */
    {section:"settings",order:46.3,label:'Aivis の先読み',value:function(){
      var st=AIVIS_PREFETCH_STATS,why=Object.keys(st.drops).map(function(k){return k+' '+st.drops[k];}).join('・');
      if(aivisPrefetchMode()==='off')return '先読みしない（設定）';
      if(!CFG.aivisStream)return '先読みしない（ストリーミング再生がOFF）';
      return '先読み '+st.started+' / 使用 '+st.hit+' / 捨てた '+st.dropped+(why?'（'+why+'）':'')
        +(st.budget?' / 枠が近く見送り '+st.budget:'')
        +(st.hit?' ／ 平均 '+Math.round(st.aheadMs/st.hit)+'ms 前に依頼'+(st.headersLate?'・応答待ち '+st.headersLate+'回':''):''); },inactive:'(未使用)'},
    ...
    segmentStatus:function(){
      var srv=aivisServerWait(),win=aivisWindowWait();
      var policy={send:'超過はAivis側の設定（クレジット）',browser:'超過はブラウザ内蔵音声',wait:'超過は待つ'}[aivisOverLimitMode()];
      return ' ／ Aivis 10回/分（'+policy+'） · '
        +(srv>0?'Aivisが停止中（あと'+(srv/1000).toFixed(1)+'秒）'
          :win>0?'この60秒の枠は使い切り（あと'+(win/1000).toFixed(1)+'秒）'
          :'送信可能');
    }
  },
  localvv: {
...
/* アドオンのタブ音声（tabCapture）の Track では、同じ Chrome で画面共有の Track は
   認識できるのに、開始から約0.15秒で network を返し続けた（v1.49.26 の記録で22回）。
...
/* 開始直後の network が続いたときの切替先。元の Track は計測と生存確認に使い続ける。 */
WebSpeechTrackEngine.prototype.useRelay=function(){
  var AC=window.AudioContext||window.webkitAudioContext,ctx=null,src=null,dest=null;
  try{
    try{ctx=new AC({sampleRate:16000});src=ctx.createMediaStreamSource(new MediaStream([this.track]));}
    catch(e){if(ctx)ctx.close().catch(function(){});ctx=new AC();src=ctx.createMediaStreamSource(new MediaStream([this.track]));}
    dest=ctx.createMediaStreamDestination();dest.channelCount=1;dest.channelCountMode='explicit';dest.channelInterpretation='speakers';
    src.connect(dest);
    var out=dest.stream.getAudioTracks()[0];
    if(!out)throw new Error('中継Trackを作れません');
    this.relay={ctx:ctx,src:src,dest:dest,track:out};this.recTrack=out;
    this.relayReady=ctx.resume().catch(function(){});
    dlog('stt','track-relay',{seat:this.seat,rate:ctx.sampleRate,channels:1,addon:this.addonTrack(),
      trackId:String(out.id||'').slice(0,8)});
    return true;
  }catch(err){
    try{if(ctx)ctx.close().catch(function(){});}catch(_){}
    dlog('stt','track-relay-FAIL',{seat:this.seat,err:String((err&&err.message)||err).slice(0,160)});
    return false;
  }
};
WebSpeechTrackEngine.prototype.dropRelay=function(){
  var r=this.relay;this.relay=null;this.relayReady=null;this.recTrack=this.track;if(!r)return;
  try{r.src.disconnect();}catch(e){}
  try{r.track.stop();}catch(e){}
  try{r.ctx.close().catch(function(){});}catch(e){}
};
WebSpeechTrackEngine.prototype.quickFail=function(){
  var addon=this.addonTrack();
  this.fail((addon?'アドオンのタブ音声では':'この共有音声では')+'、Chrome の Web Speech が認識を始められませんでした（network）',null,
    addon?'⚙→音声認識で gpt-4o-mini-transcribe などの外部APIを選んでください。'
      +'アドオンの字幕表示を使わず、HTML本体の画面共有なら Web Speech で認識できます。'
      :'ネットワークを確認するか、⚙→音声認識で外部APIを選んでください。');
};
...
WebSpeechTrackEngine.prototype.startSameTrack=function(){
  ...
  try{
    ...
    dlog('stt','track-start',{seat:this.seat,trackId:String(this.track.id||'').slice(0,8),restart:this.restarts,
      chrome:overlayChromeMajor(),sameTrack:true,relay:!!this.relay,addon:this.addonTrack()});
    return true;
  }catch(err){
...
WebSpeechTrackEngine.prototype.fail=function(message,err,hint){
  ...
  toast(message+'。マイクへは自動切替しません。'+(hint||'⚙→音声で「VB-CABLE入力」または「外部API」を選んでください。'));
};
WebSpeechTrackEngine.prototype.build=function(){
  ...
  r.onerror=function(ev){var code=(ev&&ev.error)||'';
    ...
    if(++self.quick<WEB_TRACK_QUICK_MAX)return;
    self.quick=0;
    if(self.relay||!self.useRelay())self.quickFail();
  };
  r.onend=function(){
...
/* 録音を切って送るまでの無音待ち。実測（v1.49.21・Teams）の内訳は
   無音待ち 0.9秒 → 転写 0.69〜2.33秒 → 読み上げ先頭 0.27〜0.39秒 で、
   発話の終わりから声が出るまで 1.95〜3.6秒だった。ここが唯一、こちら側で
   短くできる固定値である。短くすると1発話が複数リクエストへ割れやすくなり、
   転写の回数と料金が増える。 */
function sttSilenceHoldMs(){
  var n=parseInt(CFG.sttSilenceHoldMs,10);
  if(!isFinite(n))return 900;
  return Math.max(200,Math.min(2000,n));
}
StreamEngine.prototype.cut = function(send,reason){
  ...
  if(this.rec._fourOMeta){this.rec._fourOMeta.endedAt=Date.now();this.rec._fourOMeta.reason=reason||'silence';
    if(this.fourO)dlog('stt','4o-recording-cut',{reason:this.rec._fourOMeta.reason,seconds:this.rec._fourOMeta.seconds,
      spanMs:Date.now()-this.segStart,silenceHoldMs:sttSilenceHoldMs(),
      sinceVoiceMs:this.last?Date.now()-this.last:null,send:!!send});}
    ...
    if(route==='direct'){
      jobs.push(
        getDisplayAudioForStt().then(function(info){
          ...
          var eng=new WebSpeechTrackEngine(dispSeat,info.track,{ownsStream:info.ownsStream,stream:info.stream,fromOverlay:!!info.fromOverlay});
          engines.push(eng);eng.start();
...
var GROQ_RECO = {
  text:{ speed:['llama-3.1-8b-instant','openai/gpt-oss-20b'], quality:['openai/gpt-oss-120b','moonshotai/kimi-k2-instruct','llama-3.3-70b-versatile'],
         price:['llama-3.1-8b-instant','openai/gpt-oss-20b'], popular:['llama-3.3-70b-versatile','openai/gpt-oss-120b'] },
...
/* 原文の大きさは訳文に対する比で持つ。文字サイズを変えても比は変わらない。 */
function srcRatioNum(){ var n=parseInt(CFG.srcRatio,10); if(!isFinite(n))n=60; return Math.max(40,Math.min(100,n)); }
function applySrcRatio(){
  var n=srcRatioNum();
  document.documentElement.style.setProperty('--src-ratio', String(n/100));
  var out=$('srcRatioNow'); if(out)out.textContent=n+'%';
}
function setSrcRatio(v){
  var n=Math.max(40,Math.min(100,parseInt(v,10)||60));
  CFG.srcRatio = String(n); persistSetting("srcRatio", CFG.srcRatio);
  applySrcRatio(); $('srcRatio').value = n;
  /* タブ内字幕は profile で受け取る。拡張が居なければ何も起きない。 */
  if(globalThis.DuoExtension && DuoExtension.publishProfile)DuoExtension.publishProfile();
  renderCaptionPip();
}
...
$('srcRatio').oninput = function(){ setSrcRatio($('srcRatio').value); };
...
var DIAG_ROWS = [
  ...
  {section:"settings",order:29.1,label:'発話交代の判断層',value:function(ctx){
    var mode=CFG.turnDecisionMode||'off';
    if(mode==='off')return 'OFF（境界判定と読み上げ開始は v1.47.1 と同一）';
    return mode+' / 英語='+(CFG.turnDecisionLangEn||'shadow')+' / 日本語='+(CFG.turnDecisionLangJa||'shadow')
      +' / 音響特徴'+(CFG.turnDecisionProsody?'あり':'なし')+' / 直近ターン'+(CFG.turnDecisionContextTurns||0);
  }},
  {section:"settings",order:29.2,label:'判断の経路',value:function(ctx){
    var id=String(CFG.turnDecisionProvider||'rules'),list=TurnProviders.list(),r=null,i;
    for(i=0;i<list.length;i++)if(list[i].id===id)r=list[i];
    if(!r)return id+'（未知の経路。Rulesへ落ちます）';
    if(r.local)return r.label+'（外部送信なし）';
    var key=TurnProviders.keyFor(r.vendor);
    return r.label+' / モデル '+(CFG.turnDecisionModel||r.defaultModel||'(既定)')
      +' / キー'+(key?'設定済み '+key.length+'文字':'未設定')
      +(r.verified?'':' / リクエスト形式は未検証');
  }},
  {section:"settings",order:29.3,label:'判断の内訳',value:function(ctx){
    var c=SEG.committedBySource||{},x=SEG.correctedBySource||{};
    return '確定 rules '+(c.rules||0)+' / provider '+(c.provider||0)
      +'　確定後訂正 rules '+(x.rules||0)+' / provider '+(x.provider||0);
  }},
  /* 「呼べたか」ではなく「使える答えが返って適用できたか」。送信数だけでは
     解析で落ちたのか古くて捨てたのか分からず、実機の切り分けができない。 */
  {section:"settings",order:29.4,label:'判断層の往復',value:function(ctx){
    var s=TurnDecision.stats||{};
    return '送信 '+(s.requested||0)+' / 正常応答 '+(s.answered||0)
      +' / 解析失敗 '+(s.parseFailed||0)+' / 古くて破棄 '+(s.stale||0)
      +' / 到達不能 '+(s.unreachable||0)+' / 適用 '+(s.applied||0)
      +'　保持tick '+(s.holdTicks||0)+' / 打切り '+(s.holdExpired||0)
      +'　句点で待たず '+(s.markNoWait||0);
  }},
...
/* ── Turn decision layer ───────────────────────────────────────────────────
   ...
   INV-11 判断層が commit を止めていられるのは turnDecisionHoldMaxWaitMs（既定2000ms）
          まで。HOLD／言い直し待ち／位置確認のいずれであっても、確定済みprefixごとの
...
/* 拡張のバックグラウンドへの中継。
   ...
   ページは別オリジンへ投げても、サーバが CORS 応答ヘッダを出さなければ結果を
   読めない。api.typesafe.ai は実測で「API は動くが Access-Control-Allow-Origin
   を出さない」だった。つまりブラウザからの直叩きは成立しない。拡張は利用者が
   許可したホストに対してこの制約を受けないので、往復だけを拡張へ肩代わりさせる。

   2通りある。拡張版（app.html）は chrome.runtime へ直接。HTML版は MAIN world から
   chrome.* が見えないので、ポップアップで接続したときに張られる内容スクリプトへ
   CustomEvent で渡す。鍵も本文もこのブラウザの中しか通らない。

   鍵が DOM イベントに載る点について: 同じページの他スクリプトからは読める。ただし
   鍵はもともと同じページの localStorage にあるので、そこを読める相手はイベントを
   待つまでもない。露出は増えていない。 */
var TurnBridge={
  /* 中継先が黙って消えることがある（タブを閉じた、拡張を更新した）。
     signal を渡さない probe もここで必ず終わる。 */
  timeoutMs:35000,
  seq:0,
  pending:{},
  ready:false,
  ...
  mode:function(){
    if(typeof chrome!=='undefined'&&chrome&&chrome.runtime&&chrome.runtime.id
      &&typeof chrome.runtime.sendMessage==='function')return 'runtime';
    return this.ready?'event':'';
  },
  available:function(){return !!this.mode();},
  wire:function(){
    if(this.wired||typeof window==='undefined'||!window.addEventListener)return;
    this.wired=true;
    var self=this;
    ...
    /* 内容スクリプトが先に張られている場合に備えて、こちらからも名乗る。 */
    try{window.dispatchEvent(new CustomEvent('duo-turn-hello'));}catch(err){}
  },
  failAll:function(reason){
    var k,p=this.pending;this.pending={};
    for(k in p)if(Object.prototype.hasOwnProperty.call(p,k))
      p[k]({ok:false,error:reason,unreachable:true});
  },
  /* fetch と同じ {status,text} を返す。上位は経路差を見ない。 */
  send:function(url,headers,body,signal){
    var m=this.mode();
    if(!m)return Promise.reject(this.bridgeError(
      'HTML本体がアドオンに接続されていません（ポップアップの「HTML本体を開く」で接続します）'));
  ...
  settleWith:function(signal,work){
    var self=this;
    return new Promise(function(resolve,reject){
      var done=false,onAbort=null,timer=null;
      function stop(){
        if(timer)clearTimeout(timer);
        if(onAbort&&signal&&signal.removeEventListener)signal.removeEventListener('abort',onAbort);
      }
      function ok(v){if(done)return;done=true;stop();resolve(v);}
      function ng(e){if(done)return;done=true;stop();reject(e);}
      onAbort=function(){var e=new Error('中断しました');e.name='AbortError';ng(e);};
      if(signal){
        if(signal.aborted){onAbort();return;}
        if(signal.addEventListener)signal.addEventListener('abort',onAbort);
      }
      timer=setTimeout(function(){ng(self.bridgeError('時間内に応答がありません'));},
        self.timeoutMs);
  ...
  viaEvent:function(req,signal){
    ...
    var drop=function(){delete self.pending[id];};
    task.then(drop,drop);
    return task;
  },
  settle:function(reply,resolve,reject){
    if(!reply){reject(this.bridgeError('アドオンから応答がありません'));return;}
    if(reply.ok){resolve({status:Number(reply.status)||0,text:String(reply.text||'')});return;}
  ...
  bridgeError:function(msg){
    var e=new Error('アドオンを経由できません: '+String(msg||'').slice(0,100));
    e.needsBridge=true;e.unreachable=true;return e;
  }
};
TurnBridge.wire();
...
var TurnProviders={
  ...
  normalize:function(raw,state,meta){
    ...
    if(t&&typeof t==='object'){
      var TS=['COMPLETE','CONTINUING','SELF_REPAIR','UNKNOWN'];
      if(TS.indexOf(String(t.choice))<0)return null;
      var tc=num(t.confidence);if(tc===null)return null;
      out.turnState={choice:String(t.choice),confidence:tc,probabilities:{}};
      /* pick は P(CONTINUING) の上限で止める。ここで確率を捨てるとその枝が死ぬ。
         boundary と同じ厳しさで検査し、通らなければ応答ごと捨てる。 */
      if(t.probabilities&&typeof t.probabilities==='object'){
        var tk=Object.keys(t.probabilities),tsum=0,ti;
        for(ti=0;ti<tk.length;ti++){
          if(TS.indexOf(tk[ti])<0)return null;
          var tv=num(t.probabilities[tk[ti]]);if(tv===null)return null;
          out.turnState.probabilities[tk[ti]]=tv;tsum+=tv;
        }
        if(tk.length&&(tsum<0.5||tsum>1.5))return null;
      }
  ...
  /* 公式の契約は question ごとに instructions と criteria を持つ。criteria は
     「選択肢名 → その説明」の map で、options 配列ではない。質問IDはモデルへ
     送られないので、意味は instructions と criteria に全部書く。
     この文面が変われば実質のモデル入力が変わるので、questionSetHash で校正を
     無効化する（§9.1）。 */
  /* instructions と criteria は英語で書く。state と引用した文脈は原語のまま渡す。
     公式が「英語が最も強い言語で、CJK は受理されるが精度は同等でない」と明記して
     いるため、モデルへの指示文は英語のほうが安全である。判断対象の日本語は state
  ...
  /* 質問が名指ししていない state は送らない。

     公式の jaggedness ページが「state が判断に無関係な内容で膨らむと精度が落ちる。
     無関係な detail は distractor になる（context rot）」と明記している。実際に
     突き合わせたら、送っている17項目のうち9項目はどの質問からも参照されていなかった。
     schemaVersion / sessionId / utteranceId / revision / speakerKey / sourceLanguage /
     speechEvent / evidence、そして candidateBoundaries。最後のものは criteria 側に
     before / after として同じ本文が入っているので、丸ごと重複だった。

     許可リストは instructions と criteria の本文からバッククォート付きの識別子を
     拾って作る。手で二重管理すると必ず片方が古くなる。文面で `x` と書けば送られ、
     書かなければ送られない、という一方向の規則にする。

     内部の state はそのまま全項目を持つ。trace・local モデル・Phase 3 の床は
     これらを使うので、削るのは送信する形だけ。 */
  SENT_FIELD_RX:/`([A-Za-z_][A-Za-z0-9_]*)(?:\.([A-Za-z_][A-Za-z0-9_]*))?`/g,
  sentFields:function(){
    if(this._sent)return this._sent;
    var src=JSON.stringify(this.template)+JSON.stringify(this.HOLD_CRITERION),
        rx=new RegExp(this.SENT_FIELD_RX.source,'g'),m,map={};
    while((m=rx.exec(src))){
      if(!map[m[1]])map[m[1]]={all:false,sub:{}};
      if(m[2])map[m[1]].sub[m[2]]=true;else map[m[1]].all=true;
    }
    return (this._sent=map);
  },
  project:function(state){
    var allow=this.sentFields(),out={},k,rule,v,sub,sk;
    for(k in state){
      if(!Object.prototype.hasOwnProperty.call(state,k))continue;
      rule=Object.prototype.hasOwnProperty.call(allow,k)?allow[k]:null;
      if(!rule)continue;
      v=state[k];
      /* `tts.queueDebtMs` のように下位だけを名指しされたときは、その下位だけ送る。
         `prosody` のように丸ごと名指しされていれば全部送る（prosody は
         TURN_PROSODY_SENT で既に絞ってある）。 */
      if(rule.all||v===null||typeof v!=='object'||v instanceof Array){out[k]=v;continue;}
      sub={};
      for(sk in v)if(Object.prototype.hasOwnProperty.call(v,sk)
        &&Object.prototype.hasOwnProperty.call(rule.sub,sk))sub[sk]=v[sk];
      out[k]=sub;
    }
    return out;
  },
  ...
  questions:function(state){
    ...
    return {
      boundary_choice:{type:'choice',instructions:this.template.boundary_choice.instructions,criteria:crit},
      turn_state:{type:'choice',instructions:this.template.turn_state.instructions,
        criteria:this.template.turn_state.criteria},
      safe_to_speak:{type:'noul',instructions:this.template.safe_to_speak.instructions,
        criteria:this.template.safe_to_speak.criteria},
      repair_likelihood:{type:'noul',instructions:this.template.repair_likelihood.instructions,
        criteria:this.template.repair_likelihood.criteria}
    };
  },

  /* 公式の応答は answers を自分の質問IDで引く形。choice は choice／probabilities／
     confidence、noul は noul の値だけを持ち confidence はない。0.5 は「中くらい」
     ではなく「判断がつかない」を意味する。 */
  fromAnswers:function(raw,state,meta){
    if(!raw||typeof raw!=='object'||!raw.answers||typeof raw.answers!=='object')return null;
    var a=raw.answers,b=a.boundary_choice,t=a.turn_state,s=a.safe_to_speak,r=a.repair_likelihood;
    if(!b||b.type!=='choice'||!t||t.type!=='choice')return null;
    if(!s||s.type!=='noul'||!r||r.type!=='noul')return null;
    var out=this.normalize({
      decisionId:raw.id||raw.request_id||('d'+Date.now()),
      boundary:{choice:b.choice,confidence:b.confidence,probabilities:b.probabilities},
      turnState:{choice:t.choice,confidence:t.confidence,probabilities:t.probabilities},
      safeToSpeak:s.noul,repairLikelihood:r.noul
    },state,{provider:meta.provider,model:String(raw.model||meta.model||''),
      semantics:meta.semantics,latencyMs:meta.latencyMs,questionSetHash:meta.questionSetHash});
    /* usage を残す。レート制限が 1,200 req/min と 250,000 tok/s のアカウント単位
       なので、実測のトークン消費を見ないと同時会議数の上限が読めない。 */
    if(out&&raw.usage&&typeof raw.usage==='object')
      out.usage={inputTokens:Number(raw.usage.input_tokens)||0,
        outputTokens:Number(raw.usage.output_tokens)||0};
    return out;
  },

  /* 再試行しない status。会話内ではどれも待たず Rules へ落とすが、設定・実装の
     誤りは判断層を止めるべきなので区別する。既知の不一致として、本来 422 の
     リクエスト検証エラーが 400 で返ることがある。 */
  ...
  setKey:function(vendor,value){
    ...
    if(value)map[vendor]=String(value);else delete map[vendor];
    var s=JSON.stringify(map);
    CFG.turnDecisionKeys=s;persistSetting('turnDecisionKeys',s);
  },

  /* chat互換の経路へ渡す答えの形。公式の systemone は answers を型付きで返すが、
     chat/completions を入口にする経路では此方が形を指定しなければならない。
     fromAnswers がそのまま食える形（type を含む）にして、正規化を1本に保つ。 */
  answerSchema:function(state){
    var ids=['HOLD'],list=state.candidateBoundaries||[],i;
    for(i=0;i<list.length;i++)ids.push(list[i].id);
    var TS=['COMPLETE','CONTINUING','SELF_REPAIR','UNKNOWN'];
    var mapOf=function(keys){
      var o={type:'object',additionalProperties:false,properties:{},required:[]},k;
      for(k=0;k<keys.length;k++){o.properties[keys[k]]={type:'number'};o.required.push(keys[k]);}
      return o;
    };
    var choice=function(enumv,probs){
      return {type:'object',additionalProperties:false,required:['type','choice','confidence','probabilities'],
        properties:{type:{type:'string',enum:['choice']},choice:{type:'string',enum:enumv},
          confidence:{type:'number'},probabilities:probs}};
    };
    var noul={type:'object',additionalProperties:false,required:['type','noul'],
      properties:{type:{type:'string',enum:['noul']},noul:{type:'number'}}};
    return {type:'object',additionalProperties:false,required:['answers'],properties:{
      answers:{type:'object',additionalProperties:false,
        required:['boundary_choice','turn_state','safe_to_speak','repair_likelihood'],
        properties:{boundary_choice:choice(ids,mapOf(ids)),turn_state:choice(TS,mapOf(TS)),
          safe_to_speak:noul,repair_likelihood:noul}}}};
  },
  /* chat経路での前置き。systemone なら instructions が質問ごとに届くが、
     chat では1本の文脈にまとめる必要がある。文面は questionSetHash に入る。 */
  ENVELOPE_PROMPT:'You answer typed decision questions about a live interpreting session. '
  ...
  /* ── 経路レジストリ ───────────────────────────────────────────────────
     判断契約は Duo が所有し、経路は「宣言」で足す。1経路が書くのは
       vendor / endpoint / 認証ヘッダ / body の組み立て / 応答の取り出し
     だけで、タイムアウト・中断・サイズ上限・model pin 検査・再試行しない status・
     回線遮断は下の httpEvaluate と observe が共通で持つ。
     OpenAI や Claude が同等の判断APIを出したら、ここに1件足せば UI の選択肢にも
     診断にも自動で出る（UIは registry から作るため、追記漏れが起きない）。

     semantics は「確率の出どころ」。native_calibrated 以外は thresholds() が
     commit 側を厳しくする。校正済みの数値と同じ扱いにしてはいけない。       */
  ROUTES:{
    'jev-direct':{
      ...
      keyHint:'TypeSafe の API キー',
      /* 公式の Models ページ: 1リクエスト 64k tokens、うち state ＋最長の質問で 32k。
         32k は OpenRouter 固有の制限ではなく直叩きにも掛かる。前版で「OpenRouter は
         直接の半分」と書いていたのは私の誤読。 */
      contextNote:'64k context（state＋最長の質問で32k）',
      /* トークン数はブラウザ側で数えられないので、文字数で保守的に代理する。
         全文が CJK でも 1文字≒1トークンを超えないため、24,000文字なら 32k の
         内側に収まる。超えたら送らず Rules へ落とす。切って送ると候補の
         before/after と本文がずれ、存在しない位置を選ばせてしまう。 */
      maxBodyChars:24000,
      capabilities:{probabilitySemantics:'native_calibrated',supportsChoice:true,supportsNoul:true,
        structuredOutput:true,abortable:true,maxChoices:255,textOnly:true,local:false},
      path:'/v1/systemone',
      /* 公式リファレンスに記載が無いのに私が足していたヘッダ。ブラウザから別オリジンへ
         投げると preflight(OPTIONS) が走り、サーバが Access-Control-Allow-Headers に
         この名前を並べていなければ本リクエストは発射されず "Failed to fetch" になる。
         実在を確認できていないヘッダのために疎通を落とすのは本末転倒なので、既定では
         送らない。送る必要が判明したら true にする。 */
      extraHeaders:false,
      headers:function(key){
        var h={'Authorization':'Bearer '+key};
        if(this.extraHeaders){
          h['X-TypeSafe-Zero-Data-Retention']='true';
          h['X-TypeSafe-No-Training']='true';
        }
        return h;
      },
      body:function(state,model){
        /* questions は候補を組むので完全な state を見る。送るのは projection。 */
        return {model:model,state:TurnProviders.project(state),
          questions:TurnProviders.questions(state)};
      },
      parse:function(json,state,meta){return TurnProviders.fromAnswers(json,state,meta);}
    },

    /* 同じ契約を、拡張のバックグラウンド経由で投げる。実測で api.typesafe.ai は
       CORS 応答ヘッダを出さないため、ブラウザからの直叩きでは結果を読めない。
       リクエストの形は jev-direct と同一で、変えているのは運び方だけ。 */
    'jev-extension':{
      label:'Jev — アドオン経由（CORS回避）',
      vendor:'typesafe',
      verified:true,
      semantics:'native_calibrated',
      transport:'extension',
      ...
      defaultModel:'jev-1.13.0',
      keyHint:'TypeSafe の API キー',
      contextNote:'64k context（state＋最長の質問で32k）・アドオンが往復を肩代わりします',
      maxBodyChars:24000,
      capabilities:{probabilitySemantics:'native_calibrated',supportsChoice:true,supportsNoul:true,
        structuredOutput:true,abortable:true,maxChoices:255,textOnly:true,local:false},
      path:'/v1/systemone',
      extraHeaders:false,
      /* 形は jev-direct と同一。2箇所に書くと必ず片方だけ古くなるので、
         関数そのものを共有する。 */
      headers:function(key){return TurnProviders.ROUTES['jev-direct'].headers.call(this,key);},
      body:function(state,model){return TurnProviders.ROUTES['jev-direct'].body(state,model);},
      parse:function(json,state,meta){return TurnProviders.fromAnswers(json,state,meta);}
    },

    'jev-openrouter':{
      label:'Jev — OpenRouter 経由',
      vendor:'openrouter',
      /* 公式の chat/completions 互換という前提で組んである。TypeSafe が
         OpenRouter 上でどの入口を出しているかは未確認なので、実機で疎通を
         確かめるまで verified:false のままにする。UIにもそう出す。 */
      verified:false,
      semantics:'vendor_reported',
      defaultBase:'https://openrouter.ai/api',
      defaultModel:'typesafe/jev',
      keyHint:'OpenRouter の API キー',
      /* 前版の「OpenRouter は直接の半分」は誤り。公式が 32k と書いているのは
         「state ＋最長の質問」の枠で、直叩きにも同じように掛かる。OpenRouter 側の
         実際の枠は確認できていないので、直叩きと同じ保守値を置いて未確認と出す。 */
      contextNote:'文脈の上限は未確認（直叩きと同じ保守値で運用）',
      maxBodyChars:24000,
      ...
      body:function(state,model){
        /* answerSchema と questions は候補を組むので完全な state を見なければならない。
           projection を先に当てると candidateBoundaries が消え、enum が HOLD だけに
           なる。「モデルは省いた値を選べない」のだから、それは全部 HOLD を強制する
           のと同じ。送るのは projection、組み立てに使うのは完全な state。 */
        return {model:model,temperature:0,
          response_format:{type:'json_schema',json_schema:{name:'duo_turn_answers',strict:true,
            schema:TurnProviders.answerSchema(state)}},
          messages:[{role:'system',content:TurnProviders.ENVELOPE_PROMPT},
            {role:'user',content:JSON.stringify({state:TurnProviders.project(state),
              questions:TurnProviders.questions(state)})}]};
      },
      parse:function(json,state,meta){
        var msg=json&&json.choices&&json.choices[0]&&json.choices[0].message;
        var text=msg&&typeof msg.content==='string'?msg.content:'';
        if(!text)return null;
        var inner;try{inner=JSON.parse(text);}catch(err){return null;}
        if(!inner||typeof inner!=='object')return null;
        /* usage の綴りが OpenAI 互換側は prompt/completion。契約側の名前へ寄せる。 */
        if(json.usage&&typeof json.usage==='object')
          inner.usage={input_tokens:json.usage.prompt_tokens,output_tokens:json.usage.completion_tokens};
  ...
  /* 送信の口はここ1つ。経路が transport:'extension' なら拡張のバックグラウンドへ
     肩代わりさせる。返す形は fetch と同じ {status,text} に揃え、上位が運び方の
     違いを見ないで済むようにする。 */
  send:function(route,url,headers,body,signal){
    if(route&&route.transport==='extension')
      return TurnBridge.send(url,headers,body,signal);
    return fetch(url,{method:'POST',signal:signal,cache:'no-store',headers:headers,body:body})
      .then(function(r){
        return r.text().then(function(t){return {status:r.status,text:String(t||'')};},
          function(){return {status:r.status,text:''};});
      },function(err){
        /* status を持たない失敗は、応答が返らなかったということ。ブラウザは CORS でも
           DNS でも遮断でも同じ "Failed to fetch" を投げるので、ここで印を付ける。 */
        if(err&&err.name==='AbortError')throw err;
        if(err&&err.status)throw err;
        var e=new Error('応答が返りません（CORS・DNS・ネットワーク遮断のいずれか）');
  ...
  /* 経路を安全に引く。TurnProviders 自身のメソッド名（normalize 等）を
     provider 名に入れられると、関数がそのまま経路として扱われてしまう。
     registry を分けたうえで hasOwnProperty で引き、それ以外は必ず null を返す。 */
  get:function(id){
    var key=String(id||'');
    if(key==='local')return this.local;
    if(Object.prototype.hasOwnProperty.call(this.ROUTES,key))return this.httpProvider(key);
    return null;
  },
  ...
  list:function(){
    ...
    for(k in this.ROUTES)if(Object.prototype.hasOwnProperty.call(this.ROUTES,k))
      out.push({id:k,label:this.ROUTES[k].label,vendor:this.ROUTES[k].vendor,
        ...
        bridge:this.ROUTES[k].transport==='extension',
        contextNote:this.ROUTES[k].contextNote,defaultModel:this.ROUTES[k].defaultModel});
    return out;
  },

  /* 経路へ最小の1回だけ投げて、到達できるかを見る。判断層のモードには依存しない。
     本文は実際の契約と同じ形にする。形が違えば通っても意味がないため。 */
  probe:function(id){
    ...
    var state={schemaVersion:TURN_STATE_SCHEMA,sessionId:'probe',utteranceId:'probe',revision:0,
      ...
      stablePrefixChars:28,lastDeltaMs:400,silenceMs:600,sttFinal:true,speechEvent:'stopped',
      candidateBoundaries:[{id:'C_FULL_28',kind:'C_FULL',offset:28,
        left:'connectivity check.',right:''}],
      prosody:null,recentTurns:[],
      tts:{active:false,queueDebtMs:0,pendingFirstAudio:false,overlapMode:'auto'},
      evidence:{meterAvailable:true,partialAvailable:false,speakerReliable:false,prosodyAvailable:false}};
    var headers={'Content-Type':'application/json'},h=route.headers(key),hk;
    for(hk in h)if(Object.prototype.hasOwnProperty.call(h,hk))headers[hk]=h[hk];
    var t0=Date.now();
    return this.send(route,base.replace(/\/+$/,'')+route.path,headers,
      JSON.stringify(route.body(state,model))).then(function(r){
      var ms=Date.now()-t0;
      if(r.status<200||r.status>=300)
        return {ok:false,ms:ms,status:r.status,detail:String(r.text||'').slice(0,160)};
      /* 200 が返っただけでは使えるとは言えない。本番と同じ parse と正規化を
         通し、契約に載る答えが取り出せたときだけ ok にする。ここを緩めると
      ...
      return {ok:false,unreachable:!!(err&&err.unreachable),
        needsBridge:!!(err&&err.needsBridge),needsSetup:!!(err&&err.needsSetup),
        ms:Date.now()-t0,status:Number(err&&err.status)||0,
        detail:String((err&&err.message)||err).slice(0,120)};
    });
  },

  /* 経路の宣言から provider を作る。共通の検査と組み立てはここだけに置く。 */
  httpProvider:function(id){
    var route=this.ROUTES[id];
    return {
      id:id,route:route,
      capabilities:function(){return route.capabilities;},
      evaluate:function(state,opts){
        ...
        /* 判断を採用するモード（assist／active）では alias ではなく校正済み version を
           pin する（§8.5）。alias は新版が出ると勝手に動き、こちら側の変更なしに答えが
           変わる。公式の Models ページでは jev-latest も jev-preview も jev-1.13.0 を指す。
           経路の既定を固定versionにしたので、ここに来るのは自分で alias を入れたときだけ。 */
        if(TurnDecision.applies(state.sourceLanguage)&&/latest|preview/i.test(model))
          return Promise.reject(new Error('assist／active では model に固定versionを指定してください'
            +'（いまは '+model+'。例: jev-1.13.0）'));
        var body;try{body=JSON.stringify(route.body(state,model));}catch(err){
          return Promise.reject(new Error('リクエストを組み立てられません'));}
        /* 経路ごとの文脈上限。超えたら送らずRulesへ落とす。途中で切ると
           候補の before/after と本文がずれ、存在しない位置を選ばせてしまう。
           名前が maxStateChars だった頃から実際に見ているのは body 全体で、
           state ＋全質問なので公式の「state ＋最長の質問」より厳しい側に外れる。 */
        if(route.maxBodyChars&&body.length>route.maxBodyChars)
          return Promise.reject(new Error('この経路の文脈上限を超えました'));
        var headers={'Content-Type':'application/json'},h=route.headers(key),hk;
        for(hk in h)if(Object.prototype.hasOwnProperty.call(h,hk))headers[hk]=h[hk];
        var t0=Date.now();
        return TurnProviders.send(route,base.replace(/\/+$/,'')+route.path,headers,body,
          opts&&opts.signal).then(function(r){
          if(r.status<200||r.status>=300){var e=new Error('HTTP '+r.status);e.status=r.status;throw e;}
          if(r.text.length>200000)throw new Error('応答が大きすぎます');
          var j;try{j=JSON.parse(r.text);}catch(err){throw new Error('JSONではない応答');}
          return route.parse(j,state,{provider:id,model:model,semantics:route.semantics,
            latencyMs:Date.now()-t0,questionSetHash:TurnDecision.questionSetHash()});
        });
      }
    };
  },
...
/* 床で safe_to_speak を使うときの2つの定数。どちらも仮値で、trace が取れたら
   校正する。TURN_FLOOR_HINT_MS は答えの鮮度、TURN_FLOOR_SAFE_MAX は
   「はっきり遮る」と見なす境目（noul の 0.5＝判断がつかない、より下に置く）。 */
var TURN_FLOOR_HINT_MS=2000;
var TURN_FLOOR_SAFE_MAX=0.25;
...
var TurnDecision={
  ...
  /* 全体モードは「上限」であって、単なる on/off ではない。以前は言語別の設定が
     全体を飛び越えられた。全体を shadow にしても、日本語を active にしていれば
     日本語だけ動作へ反映されてしまう。親のつまみを絞ったのに子が効き続ける形で、
     「shadow なら動作は変わらない」という前提が崩れる。小さいほうを採る。 */
  RANK:{off:0,shadow:1,assist:2,active:3},
  modeFor:function(lang){
    var g=String(CFG.turnDecisionMode||'off');
    if(g==='off')return 'off';
    var l=String(lang||'').split('-')[0],per='off';
    if(l==='en')per=String(CFG.turnDecisionLangEn||'shadow');
    else if(l==='ja')per=String(CFG.turnDecisionLangJa||'shadow');
    else return 'off';
    var rg=this.RANK[g],rp=this.RANK[per];
    if(rg===undefined||rp===undefined)return 'off';
    return rp<=rg?per:g;
  },
  ...
  /* assist は「待て」だけを採り、切る位置は Rules のままにする。
     実測（英語24件・日本語25件）で Jev が Rules より前で切れと言った回数は0で、
     価値は待つ側に全部あった。外したときの被害も「少し待つ」だけで、
     turnFloorMaxWaitMs で頭打ちになる。active は位置も採る。 */
  holdOnly:function(lang){return this.modeFor(lang)==='assist';},
  ...
  /* 同じ文の同じ revision を何度も聞かない。bucket は無音を100ms刻みで量子化するので、
     テキストが凍ったまま無音だけが伸びる状態は「新しいstate」として通り続ける。
     v1.49.19の実測では、そうして内容の同じ往復（revision:1／37文字）が187秒で433件、
     Realtime（revision:85／78文字）では406秒で1251件に達していた。revision は新しい
     テキストが来たときだけ進むので、聞く回数は revision で数えるのが正しい。 */
  maxAsks:function(){
    var n=parseInt(CFG.turnDecisionMaxAsksPerRevision,10);
    if(!isFinite(n)||n<1)return 1;
    return Math.min(20,n);
  },
  asksExhausted:function(state){
    var a=this._asks||(this._asks={}),k=this.key(state),e=a[state.speakerKey];
    if(!e||e.key!==k){a[state.speakerKey]={key:k,n:1,capped:false};return false;}
    if(e.n>=this.maxAsks()){
      if(!e.capped){e.capped=true;
        dlog('segment','turn-decision-ask-capped',{utteranceId:state.utteranceId,
          revision:state.revision,speakerKey:state.speakerKey,asks:e.n,
          textChars:state.currentText.length,limit:this.maxAsks()});}
      return true;
    }
    e.n++;return false;
  },
  ...
  /* 答え1件を1回だけ数える。cache は同じ答えを80ms周期で読み直すので、tick で
     ...
     Realtime は送信1251／適用3487）。保持している tick 数は holdTicks で別に持つ。 */
  countApplied:function(hit){
    if(!hit||hit.appliedCounted)return;
    hit.appliedCounted=true;this.bump('applied');
  },
  ...
  markCut:function(text,ruleResult){
    var n=ruleResult&&ruleResult.length;
    if(!n||!this.commitWaitMs())return false;
    return /(?:[。！？!?]|[^.\d]\.)[\s」』）"'’”)\]]*$/.test(String(text||'').slice(0,n));
  },
  ...
  /* INV-11。判断層が commit を止めていられる時間にも上限を置く。INV-09 は床、
     INV-10 は答えの到着だけを縛っていて、返ってきた HOLD そのものには上限が
     無かった。revision は新しいテキストが来たときしか進まないので、STTが止まった
     瞬間の HOLD は同じ答えを読み直し続け、そのカードは commit も翻訳も読み上げも
     されないまま停止まで残る（v1.49.19実測 stabilityMs 112557／83967、どちらも
     最後に audio cancelled（speech-stopped））。時計は commit 済みprefixごとに
     持つので、テキストが伸びても止まらない。打切り後は同じprefixのあいだ静かに
     Rules へ通す。

     時計を止めるのは commit したときだけ。答えが無くて Rules がそのまま通った tick
     では止めない。止めてしまうと、Realtime のように新しいテキストが200msごとに来る
     経路では「答え待ちの tick」が毎回時計を戻し、HOLD が版をまたいで続くあいだ
     カードが伸び続ける（v1.49.19実測で501文字・8部分のカード）のを縛れない。 */
  holdMaxWaitMs:function(){
    var n=parseInt(CFG.turnDecisionHoldMaxWaitMs,10);
    if(!isFinite(n)||n<0)return 2000;
    return Math.min(10000,n);
  },
  holdClear:function(e,out){
    var b=e&&e.segment;if(b){b.holdSince=0;b.holdOver=false;}
    return out;
  },
  holdBounded:function(e,s,state,out,ruleResult,now){
    var b=e&&e.segment;if(!b)return out;
    var anchor=(s&&s.start)||0,ms=this.holdMaxWaitMs();
    if(b.holdStart!==anchor){b.holdStart=anchor;b.holdSince=0;b.holdOver=false;}
    if(b.holdOver)return ruleResult;
    if(!ms){b.holdOver=true;this.bump('holdExpired');return ruleResult;}
    if(!b.holdSince){b.holdSince=now;
      dlog('segment','turn-decision-hold',{cardId:e.id,utteranceId:state.utteranceId,
        revision:state.revision,why:out.waiting,maxWaitMs:ms});}
    this.bump('holdTicks');
    if(now-b.holdSince<ms)return out;
    b.holdOver=true;this.bump('holdExpired');
    dlog('segment','turn-decision-hold-expired',{cardId:e.id,utteranceId:state.utteranceId,
      revision:state.revision,why:out.waiting,waitedMs:now-b.holdSince,
      ruleLength:(ruleResult&&ruleResult.length)||0});
    return ruleResult;
  },
  ...
  observe:function(state){
    ...
    if(id==='rules')return;
    /* registry からだけ引く。TurnProviders のメソッド名を provider に入れられても
       経路として扱わない。 */
    var p=TurnProviders.get(id);
    ...
    dlog('segment','turn-decision-request',{provider:id,
      model:String(CFG.turnDecisionModel||'')||(routeInfo&&routeInfo.defaultModel)||'',language:lang,
    ...
    Promise.resolve(p.evaluate(state,{signal:ctrl&&ctrl.signal,deadlineMs:timeoutMs}))
      .then(function(norm){
        ...
        if(!norm){self.circuitFail(id,lang,'schema');self.bump('parseFailed');
        ...
        dlog('segment','turn-decision-result',{latency:norm.latencyMs,choice:norm.boundary.choice,
          ...
          inputTokens:norm.usage?norm.usage.inputTokens:null});
      },function(err){
        done();
        var status=err&&err.status,reason=status?('http-'+status):String((err&&err.message)||err).slice(0,80);
        /* 再試行しない status は設定か実装の誤り。自動再試行せず判断層を止める。
           429／529／5xx は会話内では待たず Rules へ落ち、circuit breaker が扱う。 */
        if(err&&err.unreachable)self.bump('unreachable');
        if(err&&err.unreachable&&!self._toldUnreachable){
          self._toldUnreachable=true;
          toast(err.needsBridge?('判断層へ到達できません。<br>'+err.message
              +'<br>Rules で動作を続けます。')
  ...
  /* 言語別の初期閾値。校正前の出荷値であり、§9.1 の学習で置き換える。
     boundary の判定を confidence ではなく probabilities に置いているのが要点。
     公式の confidence は選択肢数 N に依存する統計量で、boundary_choice の N は
     HOLD＋候補1〜5件で毎回変わる。同じ p_max でも候補を多く作った回ほど
     confidence が高く出るため、固定の下限を置くと「Duo が何件候補を作ったか」で
     合否が変わってしまう。turn_state は選択肢が4つ固定なので confidence を使える。 */
  thresholds:function(lang,semantics){
    var l=String(lang||'').split('-')[0],t;
    if(l==='ja')t={holdMax:0.10,chosenMin:0.80,turnConfMin:0.50,
      continuingMax:0.10,repairMax:0.12,silenceMin:450,confirm:2};
    else t={holdMax:0.15,chosenMin:0.70,turnConfMin:0.50,
      continuingMax:0.15,repairMax:0.20,silenceMin:250,confirm:1};
    /* 閾値は native_calibrated な確率の上で決めた値である。経路を変えると確率の
       出どころが変わり、同じ数値が同じ意味を持たなくなる。校正されていない確率を
       そのまま同じ閾値に通すと、確信度の水増しをそのまま採用してしまう。
       校正外の経路は commit 側だけ厳しくし、待つ側（HOLD）は緩めない。 */
    if(semantics&&semantics!=='native_calibrated'){
      t=({holdMax:t.holdMax,chosenMin:Math.min(0.95,t.chosenMin+0.10),
        turnConfMin:Math.min(0.95,t.turnConfMin+0.10),
        continuingMax:t.continuingMax,repairMax:t.repairMax,
        silenceMin:t.silenceMin,confirm:Math.max(2,t.confirm+1),uncalibrated:true});
    }
    return t;
  },

  /* ヒステリシス。同じ offset をN回連続で支持したときだけ commit する。
     閾値付近で判定が振動しても、一度の揺れで確定させない。 */
  confirmed:function(state,offset,need){
    if(!(need>1))return true;
    var c=this._confirm||(this._confirm={}),e=c[state.speakerKey];
    if(!e||e.offset!==offset||e.utteranceId!==state.utteranceId||e.sessionId!==state.sessionId){
      c[state.speakerKey]={offset:offset,utteranceId:state.utteranceId,sessionId:state.sessionId,n:1};
      return false;
    }
    e.n++;return e.n>=need;
  },

  /* 候補外のoffsetは採用しない。安定prefixを越えるものも採らない。そのうえで
     確率・音響・期限を合成する。null を返すと Rules の結果がそのまま通る。 */
  pick:function(state,hit,ruleResult,holdOnly){
    var b=hit&&hit.boundary;
    if(!b||!b.choice)return null;
    /* 確率の出どころで閾値を変える。hit.probabilitySemantics は adapter が申告する。 */
    var th=this.thresholds(state.sourceLanguage,hit&&hit.probabilitySemantics),
        probs=b.probabilities||{},i;
    var num=function(v){return typeof v==='number'&&isFinite(v)?v:null;};
    var pHold=num(probs.HOLD);

    /* HOLD を選んだ、または HOLD の確率が上限を超えるなら待つ。 */
    if(b.choice==='HOLD'||(pHold!==null&&pHold>th.holdMax)){
      this.countApplied(hit);
      return {length:0,reasons:[],waiting:'provider-hold',source:'provider'};}

    var list=state.candidateBoundaries||[],cand=null;
    for(i=0;i<list.length;i++)if(list[i].id===b.choice)cand=list[i];
    if(!cand||cand.offset>state.stablePrefixChars)return null;

    /* 選ばれた候補そのものの確率で見る。候補数に依存しない量。 */
    ...
    /* assist は位置を採らないので、ここから先は要らない。位置を確定させるための
       ...
       だけになる。null を返して Rules の位置をそのまま通す。 */
    if(holdOnly)return null;

    /* 現行ruleが既に同じ位置を選んでいるなら確認は要らない。 */
    var agrees=ruleResult&&ruleResult.length===cand.offset;
    if(!agrees&&!this.confirmed(state,cand.offset,th.confirm))
  ...
  boundary:function(e,s,input,ruleResult,silence,now){
    ...
    if(!hit){
      /* Rules が文末の記号で切るなら答えを待たない。句点の位置は判断層に聞いても
         変わらない（v1.49.26 の実測で、日本語383件の往復のうち判断層が切った部分は0）。
         待てば、その分だけ翻訳と読み上げが遅れる。 */
      if(this.markCut(input.text,ruleResult)){
        var k=this.key(state),rec=this.inflight[state.speakerKey];
        if(s&&rec&&rec.key===k&&s.markNoWaitKey!==k){s.markNoWaitKey=k;this.bump('markNoWait');
          dlog('segment','turn-decision-no-wait',{utteranceId:state.utteranceId,revision:state.revision,
            speakerKey:state.speakerKey,why:'sentence-mark',ruleLength:ruleResult.length});}
        return ruleResult;
      }
  ...
  /* 保持する理由が生じたときだけ時計を回す。以前は assist/active であれば
     理由の有無に関わらず時計を回し、3秒を超えた job に expiry を適用していた。
     読み上げ待ちは実測で20〜40秒に達するので、expiry=drop のままでは
     「フロアで保持した覚えのない音声」を理由なく捨てる。保持していないなら
     期限も無い。 */
  floor:function(job,silence,now,overlap){
    var e=job&&job.card;
    if(!e||!this.applies(e.srcLang)){if(job)job.floorSince=0;return null;}
    var why=this.floorHold(job,silence,now,overlap);
    if(!why){job.floorSince=0;return null;}
    if(!job.floorSince){
      job.floorSince=now;this.bump('floorHeld');
      /* 保持し始めた1回だけ書く。floor は80ms周期で呼ばれるので毎回書くと埋まる。 */
      dlog('segment','floor-hold',{cardId:e.id,seq:job.segment&&job.segment.seq,
        why:why,overlap:overlap||'',
        safeToSpeak:job.segment&&job.segment.floorHint?job.segment.floorHint.safeToSpeak:null,
        maxWaitMs:Number(CFG.turnFloorMaxWaitMs||3000)});
    }
    ...
    return {action:'hold',reason:why,waitMs:waited};
  },
  /* 床の判定そのもの。commit のときに同じ1リクエストで答えてもらった
     safe_to_speak（noul）を読む。ここで新しく問い合わせない。

     「重なるか」ではなく「重なってよいか」で分ける ――
     同時通訳は元の話者に重ねて読むのが普通で、それは segmentOverlap=allow という
     利用者の明示的な選択である。だから allow のときは床を効かせない。効くのは
     avoid と auto、つまり「無音を待つ」とルールが決めた場面だけで、そこでは
     無音の秒数より safe_to_speak のほうが確かな根拠になる。

     noul は 0.5 が「判断がつかない」。TURN_FLOOR_SAFE_MAX はその下側に置き、
     はっきり「いま鳴らすと遮る」と言ったときだけ保持する。confidence では
     閾値を切らない（noul は confidence を持たない）。 */
  floorHold:function(job,silence,now,overlap){
    if(!CFG.turnFloorSafeToSpeak)return null;
    if((overlap||'allow')==='allow')return null;
    var s=job&&job.segment,hint=s&&s.floorHint;
    if(!hint||typeof hint.safeToSpeak!=='number')return null;
    /* 答えは commit 時点の音響と待ち行列から出ている。待ち行列で数秒経っていれば
       もうその場面の話ではないので使わない。保持は自然にこの窓で頭打ちになる。 */
    if(now-hint.at>TURN_FLOOR_HINT_MS)return null;
    return hint.safeToSpeak<=TURN_FLOOR_SAFE_MAX?'safe-to-speak':null;
  },

  reset:function(why){this._toldUnreachable=false;
    var self=this;
...
/* Aivis へ文の途中で渡さないための待ちに上限を置く。待ちは「カードが1.5秒以内に
   ...
   待ちが1.2秒を超えるなら、ここまでで読み始めたほうが早い。
   時計は「その部分が読める状態になった時刻」（訳文が届いた時刻、原文読み上げなら確定時刻）
   から回す。前の音を鳴らしているあいだも続きを待っていたことになるので、鳴り終わった時点で
   1.2秒を過ぎていれば、もう待たない。dry は先読みの下見で、時計も記録も動かさない。 */
var SEG_SENTENCE_WAIT_MAX_MS=1200;
function segSentenceWaitLeft(j,now,dry){
  var s=j.segment,ready=Number(j.manual||CFG.ttsSrc?s.committedAt:s.translatedAt)||now;
  var at=j.sentenceWaitAt||Math.min(now,ready);
  if(!dry&&!j.sentenceWaitAt)j.sentenceWaitAt=at;
  var left=SEG_SENTENCE_WAIT_MAX_MS-(now-at);
  if(!dry&&left<=0&&!j.sentenceWaitCapped){j.sentenceWaitCapped=true;SEG.sentenceWaitCapped=(SEG.sentenceWaitCapped||0)+1;
    dlog('segment','tts-wait-capped',{cardId:j.card.id,segmentId:j.segment.id,reason:'sentence-boundary',waitMs:now-at});}
  return left;
}
/* Aivis に一度に送る部分の組。segPump と先読みの両方がこれを使うので、先読みした組と
...
/* ---------------- Aivis の先読み ----------------
   読み上げは1件ずつで、次の部分の合成要求は、前の音が鳴り終わってから出していた。
   Aivis は最初の音まで0.2〜0.5秒かかるので、部分が替わるたびにその分の無音が挟まる
   （190秒の記録で約15回の切り替わり）。前の音を受信し終え、鳴り終わりまで
   AIVIS_PREFETCH_LEAD_MS を切ったら、次に送る組の要求を先に出しておく。
   - 鳴らすのは今までどおり前の音が終わってから。音は重ねない。
   - 送る本文は segPump と同じ segAivisGroup で決める。送るときに本文・声・話速の
     どれか1つでも違えば、先読みした応答は使わずに捨てて頼み直す。
   - 1回の先読みは1回の要求。ただし前の音と重ならないぶん読み上げが話し手に追いつくので、
     まとめて送れる部分が減り、要求の数は増える（代役の Aivis で6文が4回→6回）。
     直近60秒に AIVIS_PREFETCH_BUDGET 回以上送っていたら先読みせず、従来どおり
     まとめて送る。429 で止められているとき、枠を使い切っているときも先読みしない。
   - 前の音の受信中は先読みしない（Aivis へ同時に2本つながない）。 */
var AIVIS_PREFETCH_LEAD_MS=1200,AIVIS_PREFETCH_MAX_AGE_MS=8000,AIVIS_PREFETCH_BUDGET=8;
var AIVIS_PREFETCH=null,AIVIS_STREAM_TAIL=null;
var AIVIS_PREFETCH_STATS={started:0,hit:0,dropped:0,aheadMs:0,headersLate:0,budget:0,drops:{}};
function aivisPrefetchMode(){return String((typeof CFG!=='undefined'&&CFG&&CFG.aivisPrefetch)||'on')==='off'?'off':'on';}
function aivisPrefetchDrop(why){
  var p=AIVIS_PREFETCH;if(!p)return;AIVIS_PREFETCH=null;
  try{if(p.ctrl)p.ctrl.abort();}catch(e){}
  /* 応答が届いた後の abort は本文を止めない。読まずに閉じる。 */
  p.promise.then(function(r){try{if(r&&r.body&&r.body.cancel)r.body.cancel().catch(function(){});}catch(e){}},function(){});
  AIVIS_PREFETCH_STATS.dropped++;AIVIS_PREFETCH_STATS.drops[why]=(AIVIS_PREFETCH_STATS.drops[why]||0)+1;
  dlog('tts','aivis-prefetch-drop',{why:why,ageMs:Date.now()-p.at,members:p.jobs.length,chars:p.chars});
}
/* 送る直前に呼ぶ。要求本文が完全に一致したときだけ先読みを渡す。 */
function aivisPrefetchTake(key){
  var p=AIVIS_PREFETCH;if(!p)return null;
  if(p.gen!==ttsGen){aivisPrefetchDrop('stopped');return null;}
  if(p.key!==key){aivisPrefetchDrop('body-changed');return null;}
  AIVIS_PREFETCH=null;
  var age=Date.now()-p.at;
  AIVIS_PREFETCH_STATS.hit++;AIVIS_PREFETCH_STATS.aheadMs+=age;if(!p.headersAt)AIVIS_PREFETCH_STATS.headersLate++;
  dlog('tts','aivis-prefetch-hit',{ageMs:age,headersReady:!!p.headersAt,headersMs:p.headersAt?p.headersAt-p.at:null,
    members:p.jobs.length,chars:p.chars});
  return p;
}
/* segPump の送り出しで、先読みした組がそのまま使えるか。本文は部分ごとに比べる。 */
function aivisPrefetchPlanned(j,useSource){
  var p=AIVIS_PREFETCH;if(!p)return null;
  if(p.jobs[0]!==j||p.useSource!==!!useSource){aivisPrefetchDrop('plan-changed');return null;}
  var ok=p.jobs.every(function(q,i){var t=useSource?q.segment.sourceText:q.segment.translationText||'';
...
/* 鳴っている部分の次に送る組を下見する。segPump が送り出す前に確かめることを、
   副作用なしで同じ順に確かめる。1つでも通らなければ先読みしない。 */
function segPrefetchPlan(now){
  var a=SEG.active;if(!a||a.manual)return null;
  SEG.queue.sort(segQueueCompare);
  var members=a.group||[a],j=null;
  for(var i=0;i<SEG.queue.length;i++){var c=SEG.queue[i];
    if(members.indexOf(c)>=0||c.cancelled||c.epoch!==SEG.epoch||c.card.segment.cancelled)continue;j=c;break;}
  if(!j||j.manual||j.forceBrowserTts||j.segment.dispatched)return null;
  var e=j.card,s=j.segment,useSource=!!CFG.ttsSrc,lang=useSource?e.srcLang:e.dstLang;
  if(lang!=='ja'||!ttsProv().segmentJapaneseBatch||!segManualJobValid(j)||segEarlierOpen(j))return null;
  if(!useSource&&(!s.translationReady||s.translationError))return null;
  if(!segAudioAllowed(e)||!(useSource?s.sourceText:s.translationText||'').trim())return null;
  if(ttsShortSkipFor({srcText:s.sourceText,dstText:s.translationText||'',srcLang:e.srcLang,dstLang:e.dstLang},useSource))return null;
  var debt=segDebt(),silence=segSilence(e,now),overlap=segOverlapFor(e,debt);
  if(overlap==='avoid'&&!e.segment.final&&(silence===null||silence<segPolicyFor(e,debt).silence))return null;
  if(TurnDecision.applies(e.srcLang)&&TurnDecision.floorHold(j,silence,now,overlap))return null;
  if(aivisFallbackWait()>0||aivisRateWait()>0||ttsRateWaitMs()>0)return null;
...
function aivisPrefetchStart(plan,now){
  var prev=SEG.dispatchRate,body;SEG.dispatchRate=plan.rate;
  ...
  p.promise.then(function(){p.headersAt=Date.now();},function(){});
  AIVIS_PREFETCH=p;AIVIS_PREFETCH_STATS.started++;
  dlog('tts','aivis-prefetch',{cardId:plan.jobs[0].card.id,members:plan.jobs.map(function(q){return q.segment.id;}),
    chars:plan.text.length,remainMs:AIVIS_STREAM_TAIL?Math.round(AIVIS_STREAM_TAIL.remainMs()):null,rate:plan.rate});
}
/* segTick から毎回呼ぶ。古くなった先読みを捨て、条件がそろえば1件だけ先読みする。 */
function aivisPrefetchTick(now){
  var p=AIVIS_PREFETCH;
  if(p){
    var why=p.gen!==ttsGen?'stopped':p.epoch!==SEG.epoch?'reset':now-p.at>AIVIS_PREFETCH_MAX_AGE_MS?'expired':
      (SEG.queue.indexOf(p.jobs[0])<0||p.jobs[0].segment.dispatched)?'unused':aivisPrefetchMode()==='off'?'off':'';
    if(why)aivisPrefetchDrop(why);
    return;
  }
  if(aivisPrefetchMode()==='off')return;
  var t=AIVIS_STREAM_TAIL;
  if(!SEG.active||!t||t.gen!==ttsGen||!t.recvDone||t.remainMs()>AIVIS_PREFETCH_LEAD_MS)return;
  var plan=segPrefetchPlan(now);
  if(!plan)return;
  aivisWindowWait();                         /* 60秒より古い記録を落としてから数える */
  if(AIVIS_RATE.sent.length>=AIVIS_PREFETCH_BUDGET){
    if(SEG.active.prefetchBudget!==true){SEG.active.prefetchBudget=true;AIVIS_PREFETCH_STATS.budget++;
      dlog('tts','aivis-prefetch-skip',{why:'budget',count60s:AIVIS_RATE.sent.length,budget:AIVIS_PREFETCH_BUDGET,limit:10});}
    return;
  }
  aivisPrefetchStart(plan,now);
}
function segPump(){
  ...
  while(SEG.queue.length){
    ...
    var floor=TurnDecision.floor(j,silence,Date.now(),overlap);
    ...
    if(ttsProv().segmentJapaneseBatch&&lang==='ja'&&!j.forceBrowserTts){
      /* ここで逃がすのは、こちら側の数え（＝予測）では無く aivisOverLimit が
         ...
         既定（send）では予測で止めないので、超過分は Aivis 側の設定＝クレジットに
         任せる。逃がすときに待たないのは、待つ設計では実測で TTS 待ちが20〜40秒に
         達していたから。逃がすあいだは Aivis 用のまとめ処理（同一声・500文字まで）
         を飛ばす。まとめるのは Aivis の枠を節約するためなので、その枠を使わない
         のなら意味が無い。 */
      var rateWait=aivisFallbackWait();
      if(rateWait>0){
        j.forceBrowserTts=true;
        dlog('segment','tts-rate-fallback',{cardId:e.id,seq:s.seq,
          why:aivisServerWait()>0?'aivis-server-limit':'aivis-window-limit',
          overLimit:aivisOverLimitMode(),
          waitMs:rateWait,count60s:AIVIS_RATE.sent.length,limit:10});
      }
      /* 先読みした組がまだ有効なら、その組のまま送る。あとから続きが届いて組が
         伸びるとしても、合成済みの要求を捨てて頼み直すより早く、回数も増えない。 */
      if(j.forceBrowserTts)aivisPrefetchDrop('browser-fallback');
...
function segAudioEvent(msg,data){
  ...
  if(/^(browser-start|wa-play|rate-play|play-start|aivis-first|openai-first|vv-start)$/.test(msg)&&!a.firstAudioAt){
    ...
    dlog('segment','audio-start',{cardId:j.card.id,seq:s.seq,manualReplay:!!j.manual,
...
function segTick(){
  try{
    ...
    segPump();aivisPrefetchTick(Date.now());segStatus();
...
/* 上限の扱いは2種類あり、混ぜてはいけない。
   - 予測（aivisWindowWait）：こちら側で数えた60秒10回。公表値ではあるが、上位プラン
     では超過分をクレジットで払う設定にできるので、これは「上限」ではない。
   - 事実（aivisServerWait）：Aivisが429で止めてきた時間。これは必ず従う。
   v1.49.10は予測で送信を止めてブラウザ内蔵音声へ回していたため、超過分の課金を
   許可しているアカウントでも声が替わっていた。既定（send）では予測で止めない。 */
function aivisOverLimitMode(){
  var m=String((typeof CFG!=='undefined'&&CFG&&CFG.aivisOverLimit)||'send');
  return m==='browser'||m==='wait'?m:'send';
}
function aivisServerWait(){ aivisRateLoad(); return Math.max(0,AIVIS_RATE.blockedUntil-Date.now()); }
function aivisWindowWait(){
  ...
  return sent.length>=10?Math.max(0,sent[sent.length-10]+60100-now):0;
}
/* 送信を止めておく時間。aivisLimitedFetch がこの値で待つ。 */
function aivisRateWait(){
  return aivisOverLimitMode()==='wait'?Math.max(aivisServerWait(),aivisWindowWait()):aivisServerWait();
}
/* ブラウザ内蔵音声へ回すかどうか。待つ設定のときは回さない（v1.49.9以前と同じ）。 */
function aivisFallbackWait(){
  var mode=aivisOverLimitMode();
  if(mode==='wait')return 0;
  return mode==='browser'?Math.max(aivisServerWait(),aivisWindowWait()):aivisServerWait();
}
...
function aivisLimitedFetch(url,options,life){
  ...
  function attempt(){
    ...
    return wait().then(function(){
      if(cancelled())abort();
      aivisWindowWait();                       /* 60秒より古い記録を落としてから積む */
