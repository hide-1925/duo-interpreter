...
var APP_VERSION = 'v1.49.9';
var APP_BUILD = '20260923-v1499-openai-tts-limits';
...
function apiSpeakBlob(text, lang, seat, prosody, plan, key){
  ...
  fetch('https://api.openai.com/v1/audio/speech',{
    ...
    var m=String((err&&err.message)||err);
    dlog('tts','openai-FAIL',{err:m.slice(0,160),ms:Date.now()-t0,status:err&&err.status||0}); finish();
    ...
    if (err&&err.status===429){
      if (!openaiRateWarned){ openaiRateWarned=true;
        ...
        toast('OpenAI音声の利用上限に達しました。<br><small>'+m.slice(0,120)+'</small>'); }
      return;
    }
