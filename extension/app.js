  ...
  httpProvider:function(id){
    ...
    return {
      ...
      evaluate:function(state,opts){
        ...
        var model=String(CFG.turnDecisionModel||'')||route.defaultModel;
        /* active では alias ではなく校正済み version を pin する（§8.5）。alias は
           新版が出ると勝手に動き、こちら側の変更なしに答えが変わる。公式の
           Models ページでは jev-latest も jev-preview も jev-1.13.0 を指している。 */
        if(TurnDecision.applies(state.sourceLanguage)&&/latest|preview/i.test(model))
          return Promise.reject(new Error('active では model に固定versionを指定してください'
