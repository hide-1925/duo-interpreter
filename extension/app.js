...
function turnDecisionInstall(){
  ...
  box.innerHTML='<summary>発話交代の判断層（実験）</summary>'
    ...
    +'<label>判断モード <select id="turnDecisionMode">'
      ...
      +'<option value="assist">assist — 高確信のときだけ採用</option>'
      +'<option value="active">active — 主経路にする</option>'
    +'</select></label>'
  ...
  ['turnDecisionMode','turnDecisionLangEn','turnDecisionLangJa','turnDecisionProsody',
   'turnDecisionContextTurns','turnFloorMaxWaitMs','turnFloorExpiry',
...
var TurnDecision={
  ...
  stats:{requested:0,answered:0,parseFailed:0,stale:0,applied:0,unreachable:0},
  ...
  pick:function(state,hit,ruleResult){
  ...
  boundary:function(e,s,input,ruleResult,silence,now){
    ...
    return this.pick(state,hit,ruleResult)||ruleResult;
  },
  ...
  floor:function(job,silence,now){
    ...
    var why=this.floorHold(job,silence,now);
  ...
  /* 話者フロアの判定そのもの。Phase 3 は話者帰属の実測が前提で未実装なので、
     保持する条件をまだ持たない。ここが null を返す間、Floor Gate は無効で、
     読み上げ開始は既存の overlap ルールだけで決まる。 */
  floorHold:function(){return null;},
