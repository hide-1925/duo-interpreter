...
var APP_BUILD = '20260921-v1471-conference-handshake';
...
var TurnProviders={
  ...
  fromAnswers:function(raw,state,meta){
    ...
    var out=this.normalize({
      ...
      turnState:{choice:t.choice,confidence:t.confidence},
  ...
  'jev-direct':{
    ...
    evaluate:function(state,opts){
      ...
      /* model は必須。active では alias ではなく校正済み version を pin する（§8.5）。 */
      var model=String(CFG.turnDecisionModel||'')||'jev-latest';
      ...
      return fetch(base.replace(/\/+$/,'')+'/v1/systemone',{
        ...
        body:JSON.stringify({model:model,state:state,questions:TurnProviders.questions(state)})
      }).then(function(r){
        ...
        return TurnProviders.fromAnswers(j,state,{provider:'jev-direct',model:model,
...
var TurnDecision={
  ...
  thresholds:function(lang){
    var l=String(lang||'').split('-')[0];
    if(l==='ja')return {holdMax:0.10,chosenMin:0.80,turnConfMin:0.50,
      continuingMax:0.10,repairMax:0.12,silenceMin:450,confirm:2};
    return {holdMax:0.15,chosenMin:0.70,turnConfMin:0.50,
  ...
  pick:function(state,hit,ruleResult){
    ...
    var th=this.thresholds(state.sourceLanguage),probs=b.probabilities||{},i;
