    ...
    TurnProviders.probe(r.id).then(function(res){
      ...
      if(res.unreachable){
        toast('到達できません（'+res.ms+'ms）。<br>'
          ...
          +'DevTools の Network で OPTIONS の応答をご確認ください。');
        return;
      }
...
var APP_VERSION = 'v1.49.2';
var APP_BUILD = '20260923-v1492-mode-ceiling';
...
var TurnProviders={
  ...
  probe:function(id){
    ...
    var t0=Date.now();
    return fetch(base.replace(/\/+$/,'')+route.path,{method:'POST',cache:'no-store',
      headers:headers,body:JSON.stringify(route.body(state,model))
    }).then(function(r){
      var ms=Date.now()-t0;
      if(!r.ok)return r.text().then(function(txt){
        return {ok:false,ms:ms,status:r.status,detail:String(txt||'').slice(0,160)};
      },function(){return {ok:false,ms:ms,status:r.status};});
      /* 200 が返っただけでは使えるとは言えない。本番と同じ parse と正規化を
         通し、契約に載る答えが取り出せたときだけ ok にする。ここを緩めると
         「疎通は通ったのに会議では何も起きない」が再発する。 */
      return r.text().then(function(txt){
        var j;try{j=JSON.parse(txt);}catch(err){
        ...
        if(!norm)return {ok:false,ms:ms,status:r.status,parsed:false,
          detail:'契約の形に解析できません: '+String(txt||'').slice(0,120)};
        return {ok:true,ms:ms,status:r.status,parsed:true,
          choice:norm.boundary&&norm.boundary.choice,model:norm.model};
      });
    },function(err){
      return {ok:false,unreachable:true,ms:Date.now()-t0,status:0,
        detail:String((err&&err.message)||err).slice(0,80)};
    });
  },
  ...
  httpProvider:function(id){
    ...
    return {
      ...
      evaluate:function(state,opts){
        ...
        for(hk in h)if(Object.prototype.hasOwnProperty.call(h,hk))headers[hk]=h[hk];
        var t0=Date.now();
        return fetch(base.replace(/\/+$/,'')+route.path,{
          method:'POST',signal:opts&&opts.signal,cache:'no-store',headers:headers,body:body
        }).catch(function(err){
    ...
    Promise.resolve(p.evaluate(state,{signal:ctrl&&ctrl.signal,deadlineMs:timeoutMs}))
      .then(function(norm){
        ...
        if(err&&err.unreachable&&!self._toldUnreachable){
          ...
          toast('判断層へ到達できません。<br>ブラウザから直接この API を呼べない可能性があります'
            +'（CORS）。<br>Rules で動作を続けます。');
        }
