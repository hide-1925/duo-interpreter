...
async function injectOverlay(tabId,frameId=0,documentId) {
  ...
  if(overlayReady && pong.htmlBridgeVersion !== '1.4.0')throw new Error('字幕対象タブに旧版が残っています。対象タブを再読み込みしてください');
  if (!overlayReady) {
    ...
    if(!ready?.ok||ready.htmlBridgeVersion!=='1.4.0')throw Error('字幕レイヤーの初期化が完了していません。拡張を最新版一式に更新し、対象タブを再読み込みしてください');
  }
  return tab;
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    switch (message && message.type) {
      ...
      case 'DUO_GET_STATE':
        return { ok: true, workerVersion:'1.4.0', conference:conferencePublic(), state: await getState(), htmlUrl:await getHtmlSourceUrl(), overlay:await getOverlayStatus() };
