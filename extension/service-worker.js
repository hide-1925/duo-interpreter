...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    switch (message && message.type) {
      ...
      case 'DUO_GET_STATE':
        return { ok: true, workerVersion:'1.4.15', conference:conferencePublic(), state: await getState(), htmlUrl:await getHtmlSourceUrl(), overlay:await getOverlayStatus(), turn:await turnPermission() };
