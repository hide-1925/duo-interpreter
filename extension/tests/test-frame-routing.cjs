...
(async()=>{
 ...
 const manifest=JSON.parse(fs.readFileSync(path.join(root,'manifest.json')));assert.equal(manifest.version,'1.4.3');assert.deepEqual(manifest.content_scripts[0].js.slice(0,2),['conference-adapters/mic-provenance.js','conference-adapters/teams.js']);assert(manifest.content_scripts.every(s=>s.all_frames));tests.push('Package installs provenance before adapter in matched Teams frames');
 ...
 const response={ok:true,workerVersion:'1.4.3',conference:{error:'test-error',diagnostics:[{event:'microphone-discovery',eligibleCount:0}]},state:{},overlay:{}};
 const popupContext=vm.createContext({chrome:{runtime:{getManifest:()=>({version:'1.4.3'}),onMessage:{addListener(){}},sendMessage:async()=>response},tabs:{query:async()=>[]}},document:{getElementById:node,createElement:()=>node('link'),body:{appendChild(){}}},URL:{createObjectURL:b=>{blob=b;return 'blob:test';},revokeObjectURL(){}},Blob,console,setTimeout:()=>0,clearTimeout(){}});
